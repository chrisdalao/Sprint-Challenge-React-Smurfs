...
the difference between the two - server-side routing information is requested and brought back to the user from a server so the page is always being reloaded after a request - while on the contrary client-side routing allows developers to actually decide what the Broswer should display - The data is already there but it's just not being displayed - so there's no need to refresh the page
